...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await alert("Хадгалахад алдаа гарлаа.");
      setSaving(false);
    }
  };

  if (loading) return <div className="h-screen flex items-center justify-center bg-[#FCFBF9]"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#1A1A1A]"></div></div>;
    ...
    <div className="max-w-6xl mx-auto p-8 font-montserrat text-black">
      ...
      {showSuccess && (
        <div className="fixed bottom-12 right-12 bg-[#1A1A1A] text-white border border-black shadow-2xl px-8 py-5 rounded-[2px] flex items-center gap-4 z-50 animate-in slide-in-from-bottom-4">
          ...
          <span className="text-[10px] font-bold uppercase tracking-[0.2em]">Амжилттай хадгалагдлаа!</span>
        </div>
      )}
      ...
      <div className="flex items-center justify-between mb-12">
        ...
        <h1 className="text-2xl lg:text-4xl font-playfair font-bold text-black">Бүтээгдэхүүн засах</h1>
        <div className="w-12" />
      </div>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 space-y-8">
            ...
            {(formData.imageUrls.length > 0 || newImageFiles.length > 0) && (
              <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-4 mb-8">
                ...
                {newImageFiles.map((file, index) => (
                  <div key={`new-${index}`} className="relative group aspect-[4/5] rounded-[2px] overflow-hidden border border-black/10 shadow-sm">
              ...
              <div className="flex flex-col items-center justify-center gap-4 text-center px-6">
                ...
                {(formData.imageUrls.length === 0 && newImageFiles.length === 0) ? (
                  ...
                  <p className="text-[9px] text-black font-bold uppercase tracking-widest">
                    + Нэмэлт зураг оруулах <span className="text-black/80">({formData.imageUrls.length + newImageFiles.length} зургууд)</span>
                  </p>
                )}
              </div>
            </div>
          ...
          <div className="bg-white p-8 rounded-[2px] border border-black/10 shadow-sm space-y-8">
            <h2 className="text-[10px] font-bold text-black uppercase tracking-[0.3em]">Мэдээлэл</h2>
            <div className="space-y-8">
              <div>
                <label className="text-[9px] font-bold text-black uppercase tracking-[0.2em] mb-4 block">Бүтээгдэхүүний гарчиг (Нэр)</label>
                <input required type="text" name="name" value={formData.name} onChange={handleChange} placeholder="Жишээ: 101 Сарнайтай сагс" className="w-full bg-[#FCFBF9] border border-black/10 rounded-[2px] p-5 font-playfair text-lg text-black font-bold outline-none focus:border-[#1A1A1A] transition-all" />
              </div>
              ...
              <div className="grid grid-cols-2 gap-8">
                <div>
                  ...
                  <input required type="number" name="price" value={formData.price} onChange={handleChange} placeholder="Үнэ (₮)" className="w-full bg-[#FCFBF9] border border-black/10 rounded-[2px] p-5 font-playfair text-xl text-black font-bold outline-none focus:border-[#1A1A1A] transition-all" />
                </div>
                <div>
                  <label className="text-[9px] font-bold text-black uppercase tracking-[0.2em] mb-4 block">Хямдарсан (₮)</label>
                  <input type="number" name="discountedPrice" value={formData.discountedPrice} onChange={handleChange} placeholder="Хямдарсан (₮)" className="w-full bg-[#FCFBF9] border border-black/10 rounded-[2px] p-5 font-playfair text-xl text-rose-700 font-bold outline-none focus:border-[#1A1A1A] transition-all" />
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="lg:col-span-4 space-y-8">
          <div className="bg-white p-8 rounded-[2px] border border-black/10 shadow-sm space-y-10">
            <div>
              <h2 className="text-[10px] font-bold text-black uppercase tracking-[0.3em] mb-6">Ангилал</h2>
              <div className="flex flex-wrap gap-3">
                  {dbCategories.map(cat => {
                      const isSelected = formData.categories.includes(cat.name);
                      return (
                          <button key={cat.id} type="button" onClick={() => toggleCategory(cat.name)} className={`px-4 py-3 rounded-[2px] text-[10px] font-bold uppercase tracking-wider transition-all border cursor-pointer ${isSelected ? "bg-[#1A1A1A] border-[#1A1A1A] text-white shadow-xl" : "bg-white text-black border-black/10 hover:border-[#1A1A1A]"}`}>
                              {cat.name}
                          </button>
                      );
                  })}
            ...
            <div>
              <label className="text-[9px] font-bold text-black uppercase tracking-[0.2em] mb-6 block">Зориулалт (Occasions)</label>
              <div className="flex flex-wrap gap-3">
                {PURPOSE_OPTIONS.map(purp => (
                  <button 
                    key={purp} type="button" 
                    onClick={() => toggleItem(selectedPurposes, setSelectedPurposes, purp)}
                    className={`px-4 py-3 rounded-[2px] text-[10px] font-bold uppercase tracking-wider transition-all border cursor-pointer ${selectedPurposes.includes(purp) ? "bg-[#1A1A1A] border-[#1A1A1A] text-white shadow-xl" : "bg-white text-black border-black/10 hover:border-[#1A1A1A]"}`}
                  >
                    {purp}
                  </button>
                ))}
          ...
          <div className="bg-white p-8 rounded-[2px] border border-black/10 shadow-sm space-y-10">
            {!isGiftProduct && (
              <>
                <div>
                  <h3 className="text-[9px] font-bold text-black uppercase tracking-[0.2em] mb-6">Савалгаа</h3>
                ...
                <div>
                  <h3 className="text-[9px] font-bold text-black uppercase tracking-[0.2em] mb-6">Тоо ширхэг (Иш)</h3>
                  <input type="number" name="stemCount" placeholder="Жишээ: 51" value={formData.stemCount} onChange={handleChange} className="w-full bg-[#FCFBF9] border border-black/10 rounded-[2px] p-5 text-sm font-playfair font-bold text-black outline-none focus:border-[#1A1A1A]" />
                </div>
              </>
            )}

            <div>
              <h3 className="text-[9px] font-bold text-black uppercase tracking-[0.2em] mb-6">Өнгө сонгох</h3>
              <div className="flex flex-wrap gap-4 p-4 bg-[#FCFBF9] rounded-[2px] border border-black/10">
                {COLOR_OPTIONS.map(color => (
                  <button 
                    ...
                    className={`w-10 h-10 rounded-full border-2 transition-all active:scale-90 flex items-center justify-center cursor-pointer ${selectedColors.includes(color.name) ? "border-[#1A1A1A] scale-110 shadow-lg" : "border-transparent hover:scale-105"}`}
            ...
            <div>
              <h3 className="text-[9px] font-bold text-black uppercase tracking-[0.2em] mb-6">Хэмжээ</h3>
            ...
            <button type="submit" disabled={saving} className="w-full bg-[#1A1A1A] text-white font-bold py-6 rounded-[2px] hover:bg-black transition-all active:scale-95 disabled:bg-gray-300 shadow-2xl shadow-black/10 uppercase tracking-[0.3em] text-[11px] cursor-pointer">
